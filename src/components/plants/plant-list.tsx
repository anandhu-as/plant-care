...
//displaying plants
const PlantList = ({ token, plants, zenMode }: { token: string; plants: PlantWithLastWatered[]; zenMode: boolean }) => {
    if (plants.length === 0) {
        return (
    ...
    return (
        <ul className="space-y-4">
            {plants.map((plant, index) => (
                <PlantCard key={plant.id} token={token} plant={plant} index={index} zenMode={zenMode} />
            ))}
        </ul>
    );
};
